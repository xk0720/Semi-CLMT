...
writer = SummaryWriter('tensorboard-logs/log_3')
from networks.our_net_distill_352 import Our_Net_distill as net_distill
from networks.our_net_distill_ema import Our_Net_distill as net_distill_ema
...
def get_log_path():
    ...
    log_path = "results/kvasir/log_" + time_str + '.txt'
    return log_path
...
def plot_sim_matrix(similarity_matrix, epoch):
    ...
    # plt.show()
    # Save as a png file
    plt.savefig('plots/epoch_[{}]_myplot.png'.format(epoch))

# plot the uncertainty map derived from the teacher model
def plot_uncertainty_map(teacher, epoch, img):

    # activate dropout
    teacher.t_model.apply(apply_dropout)
    with torch.no_grad():
        T_num = 4
        _, _, w, h = img.shape
        preds = torch.zeros([T_num, 1, 2, w, h]).cuda()  # class num equals 2

        for i in range(T_num):
            noise_inputs_un = img + torch.clamp(torch.randn_like(img) * 0.1, 0, 0.2)
            prediction, _, _ = teacher.t_model(noise_inputs_un.cuda())
            preds[i] = torch.softmax(prediction, dim=1)

    preds = torch.mean(preds, dim=0)
    masks_pred_mode2_certainty = preds

    uncertainty_score = -1.0 * torch.sum(masks_pred_mode2_certainty * torch.log(masks_pred_mode2_certainty), dim=1,
                                         keepdim=False)

    uncertainty_map = uncertainty_score[0].detach().cpu().numpy()

    # Set the range of colors
    vmin, vmax = 0.0, np.log(2)
    # Create the heatmap
    heatmap = plt.imshow(uncertainty_map, cmap='coolwarm')
    heatmap.set_clim(vmin=vmin, vmax=vmax)

    heatmap.axes.get_xaxis().set_ticks([])
    heatmap.axes.get_xaxis().set_ticklabels([])

    heatmap.axes.get_yaxis().set_ticks([])
    heatmap.axes.get_yaxis().set_ticklabels([])

    # Add a colorbar
    cbar = plt.colorbar(heatmap)
    cbar.set_ticks([vmin, vmax])
    cbar.set_ticklabels(['0', 'U_max'])

    # save the plot
    plt.savefig('plots_uncertainty_map/epoch_[{}]_myplot.png'.format(epoch))
    plt.close()


def iou_similarity(score, target):  # score (N, 1, Class, H, W)  target (1, N, Class, H, W)
    smooth = 1e-6
...
def train_epoch(args, scaler, model, ema_model, device, train_loader, optimizer, epoch, Dice_CE_monai_loss, dice_similarity_criteria):
    ...
    # activate dropout
    ema_model.t_model.apply(apply_dropout)
    ...
    if args.ratio == 1.0:
        for batch_idx, (img_weak, mask_weak, img_strong, _) in enumerate(train_loader):
            ...
            mask_weak = (mask_weak > 0.5).float()
            mask_weak = mask_weak.to(device)

            supervised_loss = Dice_CE_monai_loss(input=output[:batch_size], target=mask_weak[:batch_size])
            ...
            #
            # # fully-supervised monta carlo dropout
            # T_num = 4
            # _, _, w, h = images.shape
            # preds = torch.zeros([T_num, batch_size, 2, w, h]).cuda() # class num equals 2
            # for i in range(T_num):
            #     noise_inputs_un = img_strong + \
            #                       torch.clamp(torch.randn_like(
            #                           img_strong) * 0.1, 0, 0.2)
            #     with torch.no_grad():
            #         # the output of ema teacher model
            #         prediction, _, _ = ema_model.t_model(noise_inputs_un.cuda())
            #         preds[i] = torch.softmax(prediction, dim=1)
            #
            # preds = torch.mean_teacher(preds, dim=0)
            # masks_pred_mode2_certainty = preds
            #
            # # Combined consistency loss funciton based on two papers (actually is cross entropy loss)
            ...
            #                                                     0.2)).cuda())  # add gaussian noise

            # -------------------------------------------------------------------------------------
            # 1. Option_1: cross_entropy loss based on hard label
            # positive_loss_mat = F.cross_entropy(uw_ce_output, torch.argmax(output_t, dim=1), reduction="none")

            # 2. Option_2: cross_entropy loss based on soft label
            ...
            #                                      dim=1, keepdim=False)
            #
            # # threshold = (0.75 + 0.25 * ramps.sigmoid_rampup(epoch, 200)) * np.log(2) (from original paper)
        ...
        for batch_idx, (img_weak, mask_weak, img_strong, _) in enumerate(train_loader):
            ...
            # semi-supervised monta carlo dropout
            T_num = 4
            _, _, w, h = images.shape
            unlabeled_bs = batch_size - labeled_bs
...
def test(args, model, device, test_loader, epoch):
    ...
    # Update the teacher model (traditional exponential moving average method)
    # teacher.update()
    # teacher.apply_shadow()

    return val_dice, val_mIoU
...
def build(args):
    ...
    transformation_train_strong = albumentations.Compose([
        ...
        # albumentations.GaussianBlur(blur_limit=5, p=0.5),
        albumentations.Cutout(num_holes=10, max_h_size=10, max_w_size=10, fill_value=0, p=0.8),
    ])
    
    # 3. construct an augmentation following weak/strong (based on transforms)
    ...
    train_indices, _, _ = split_ids(len(input_paths))
    
    # weak_image_input = torch.cat(weak_image_array_list, dim=0)
    # strong_image_input = torch.cat(strong_image_array_list, dim=0)
    # image_input = torch.cat([weak_image_input, strong_image_input], dim=0)
    image_input = None
    
    # train_dataloader, _, val_dataloader = dataloaders_RITE.get_dataloaders(batch_size=args.batch_size,
    #                                                                        semi_ratio=args.ratio)
    train_dataloader, _, val_dataloader = dataloaders.get_dataloaders(
        input_paths, target_paths, batch_size=args.batch_size, semi_ratio=args.ratio
    )
...
def train(args):
    ...
    for epoch in range(1, args.epochs + 1):
        try:
            ...
            # load model weight for check
            # model_path = '/root/autodl-tmp/project_kvasir/new_checkpoint/best_model_[0.125]_[0.9021].pth.tar'
            # checkpoint = torch.load(model_path, map_location='cuda')
            # model.load_state_dict(checkpoint['state_dict'])

            val_dice, val_miou = test(args, model, device, val_dataloader, epoch)
            ...
            logging.info("Epoch: [{}], Dice score: [{:.4f}], mIoU score: [{:.4f}]".format(epoch, val_dice.item(),
                                                                                          val_miou.item()))
            logging.info("Epoch: [{}], Max Dice: [{:.4f}], Max mIoU: [{:.4f}]".format(epoch, max_dice_score, max_mIoU))
...
def get_args():
    ...
    parser.add_argument("--root", type=str, default="/home/kevin/PhD-datasets/Kvasir-SEG/")
    parser.add_argument("--epochs", type=int, default=200)
    parser.add_argument("--batch-size", type=int, default=8)
    parser.add_argument("--labeled-bs", type=int, default=2)
    parser.add_argument("--ratio", type=float, default=0.8)
    parser.add_argument("--weight-decay", type=float, default=0.01)
    parser.add_argument("--learning-rate", type=float, default=4e-5, dest="lr")
    parser.add_argument("--rampup", type=int, default=1)
    parser.add_argument("--visual", type=str, default='kvasir')
