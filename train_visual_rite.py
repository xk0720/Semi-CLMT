...
from Data import dataloaders_RITE as dataloaders
...
writer = SummaryWriter('tensorboard-logs/log_2')
...
def get_log_path(args):
    ...
    log_path = "results/rite/log_lam1_{}_lam3_{}_".format(args.lambda1, args.lambda3) + time_str + '.txt'
    return log_path
...
def plot_sim_matrix(similarity_matrix, epoch):
    ...
    # plt.show()
    # Save as a png file
    plt.savefig('plots/rite/epoch_[{}]_myplot.png'.format(epoch))
    ...
    for batch_idx, (img_weak, mask_weak, img_strong, _) in enumerate(train_loader):
        ...
        if args.ratio < 1.0:
            with autocast():
                ...
                with torch.no_grad():
                    if args.backbone == 'unet':
                        ...
                        output_t, _, _ = ema_model.t_model((img_weak[labeled_bs:] +
                                                            torch.clamp(torch.randn_like(img_weak[labeled_bs:]) * 0.1,
                                                                        0,
                                                                        0.2)).cuda())
                ...
                loss = args.lambda1 * supervised_loss + consistency_loss + args.lambda3 * dice_contrastive_loss
                # loss = supervised_loss + consistency_loss
                # loss = supervised_loss + 0.5 * dice_contrastive_loss
...
def build(args):
    ...
    train_dataloader, _, val_dataloader = dataloaders.get_dataloaders(batch_size=args.batch_size, semi_ratio=args.ratio)
...
def train(args):
    ...
    for epoch in range(1, args.epochs + 1):
        try:
            ...
            print("-" * 70)
            print("Epoch: [{}], Training loss: [{:.4f}]".format(epoch, loss))
            print("Epoch: [{}], Dice score: [{:.4f}], mIoU score: [{:.4f}]".format(epoch, val_dice.item(),
                                                                                          val_miou.item()))
            print("Epoch: [{}], Max Dice: [{:.4f}], Max mIoU: [{:.4f}]".format(epoch, max_dice_score, max_mIoU))

            logpath = "/home/kevin/PycharmProjects/Semi-CLMT/results/sensitivity" + "/" + args.dataset
            os.makedirs(logpath, exist_ok=True)
            file_path = os.path.join(logpath, "log_{}_lambda1_{}_lambda3_{}.txt"
                                     .format(str(args.ratio), args.lambda1, args.lambda3))

            with open(file_path, 'a') as f:
                f.write("\nEpoch: [{}], Training loss: [{:.4f}]".format(epoch, loss))
                f.write("Epoch: [{}], Dice score: [{:.4f}], mIoU score: [{:.4f}]".format(epoch, val_dice.item(),
                                                                                          val_miou.item()))

            if val_dice > max_dice:
                if epoch >= 60:
                    # plot_uncertainty_map(ema_model, epoch, img=image_input)
...
def get_args():
    ...
    parser.add_argument("--learning-rate", type=float, default=5e-4, dest="lr")
