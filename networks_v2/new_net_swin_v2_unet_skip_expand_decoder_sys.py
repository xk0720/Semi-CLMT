...
from networks_v2.crossvit import CrossTransformer
...
class Swinv2Attention(nn.Module):
    ...
    def forward(
        ...
        """
        need to return attn_weights
        """
        self_outputs, attention_probs = self.self(hidden_states, attention_mask, head_mask, output_attentions,
                                                  attention, alpha=alpha)
...
class Swinv2SelfAttention(nn.Module):
    ...
    def forward(
        ...
        # ATTENTION: we add log-sum-exp trick to avoid the overflow (nan)
        ...
        """
        spatial attention mechanism implemented by adding attention encoder to decoder
        """
        if attention is not None:
            # in-place operation
            # attention_probs += attention
            # maybe we can set the weight of the following two terms (alpha)
            
            """
            Option: spatial attention mechanism or not
            """
            attention_probs = alpha * attention_probs + (1 - alpha) * attention
        ...
        """
        need to return attn_weights (attention_probs)
        """
        return outputs, attention_probs
...
class Swinv2Layer(nn.Module):
    def __init__(self, config, dim, input_resolution, num_heads, shift_size=0, pretrained_window_size=0, operation='+'):
        super().__init__()
    ...
    def forward(
        ...
        if y[0] == "decoder" and y[1] != None:
            ...
            need to return attn_weights
            """
            attention_outputs, attention = self.attention(
                hidden_states_windows, attn_mask, head_mask, output_attentions=output_attentions, attention=y[1], alpha=alpha
            )
        else:
            attention_outputs, attention = self.attention(
        ...
        """
        need to return attention_weights
        """
        if y[0] == "decoder" and y[1] != None:
            if self.operation == '+':
                attention = attention + y[1]

        return layer_outputs, attention
...
class Swinv2PatchMerging(nn.Module):
    ...
    def __init__(self, input_resolution: Tuple[int], dim: int, norm_layer: nn.Module = nn.LayerNorm) -> None:
        ...
        # avoid nan value when computing gradient in back-propagation
...
class Swinv2Stage(nn.Module):
    ...
    def forward(
        ...
        for i, layer_module in enumerate(self.blocks):
            ...
            need to return attention_weights
            """
            layer_outputs, attention = layer_module(hidden_states, input_dimensions,
                                                    layer_head_mask, output_attentions, y=['encoder', None])
        ...
        need to return attention_weights
        """
        return stage_outputs, x_attention_encoder
...
class Swinv2Stage_up(nn.Module):
    ...
    def forward(
        ...
        # if x_attention_encoder:
        #     attention_length = len(x_attention_encoder) # the length of the array of attention weights
        ...
        for i, layer_module in enumerate(self.blocks):
            ...
            """
            # -----------------------------
            # interleave, because of the attention weight mask
            if i % 2 == 0:
                idx = (length - 1) - i - 1
            else:
                idx = (length - 1) - i + 1
            ...
            layer_outputs, attention = layer_module(hidden_states, input_dimensions, layer_head_mask, output_attentions,
                                                    y=['decoder', x_attention_encoder[idx]], alpha=alpha)

            x_attention_decoder.append(attention)
            hidden_states = layer_outputs[0]
        ...
        if output_attentions:
            stage_outputs += layer_outputs[1:]
        return stage_outputs, x_attention_decoder
...
class Swinv2Decoder(nn.Module):
    def __init__(self, config, norm_layer=nn.LayerNorm):
        ...
        for i_layer in range(self.num_layers - 1):
            self.Crossvit_layer.append(
                CrossTransformer(sm_dim=self.config.embed_dim * 2 ** i_layer,  # [96, 2*96, 4*96]
                                 lg_dim=self.config.embed_dim * 2 ** i_layer,  # [96, 2*96, 4*96]
                                 depth=2,
                                 heads=self.config.num_heads[i_layer],  # [3, 6, 12]
                                 dim_head=(self.config.embed_dim // self.config.num_heads[i_layer]),  # 32
    ...
    def forward(
            ...
            x_attention_encoder_all_stages=None,
    ):

        """
        store attention_weights of Swin-block in decoder path
        """
        x_attention_decoder_all_stages = []
        alpha = [0.5, 0.6, 0.7]
        ...
        for i, layer_module in enumerate(self.layers_up):
            if i == 0:
                ...
                # ---------------------------------------------------------------------------
                """
                Option_1: simple concatenation
                """
                hidden_states = torch.cat([hidden_states, concat_features[3 - i]], dim=-1)
                hidden_states = self.concat_back_dim[i](hidden_states)

                """
                Option_2: cross attention mechanisms
                """
                # y = self.Crossvit_layer[3 - i]
                # a, b = y(hidden_states, concat_features[3-i])
                # x = torch.cat([a, b], -1)
                # hidden_states = self.concat_back_dim[i](x)
                # ---------------------------------------------------------------------------

                layer_head_mask = head_mask[i] if head_mask is not None else None
                layer_outputs, x_attention_decoder = layer_module(hidden_states, input_dimensions, layer_head_mask,
                                                                  output_attentions,
                                                                  x_attention_encoder=x_attention_encoder_all_stages[3 - i],
                                                                  alpha=alpha[3 - i])
                ...
                """
                add attention_weights into 'x_attention_decoder'
                """
                x_attention_decoder_all_stages.append(x_attention_decoder)

        return hidden_states, x_attention_decoder_all_stages
...
class Swinv2Encoder(nn.Module):
    ...
    def forward(
        ...
        """
        store attention_weights of Swin-block in encoder path
        """
        x_attention_encoder_all_stages = []
        ...
        for i, layer_module in enumerate(self.layers):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                """
                layer_module return attention_weight
                """
                layer_outputs, x_attention_encoder = layer_module(hidden_states, input_dimensions, layer_head_mask,
                                                                  output_attentions)
            ...
            """
            add attention_weights into 'x_attention_encoder'
            """
            x_attention_encoder_all_stages.append(x_attention_encoder)
        ...
        # for e in all_hidden_states:
        #     print(e.size())
        # raise Exception

        return (hidden_states, all_hidden_states), x_attention_encoder_all_stages
...
class SwinTransformerSys(nn.Module):
    ...
    def __init__(self, config, add_pooling_layer=True, use_mask_token=False, final_upsample="expand_first"):
        ...
        if self.final_upsample == "expand_first":
            ...
            self.output = nn.Conv2d(in_channels=config.embed_dim, out_channels=config.num_classes, kernel_size=1,
                                    bias=False)
    ...
    def forward_features(self, x, head_mask=None, bool_masked_pos=None):
        ...
        encoder_outputs, x_attention_encoder_all_stages = self.encoder(
            embedding_output,
            input_dimensions,
            # head_mask=head_mask,
        ...
        return bottleneck_output, encoder_outputs[1][:-2], x_attention_encoder_all_stages

    def forward_up_features(self, x, concat_feature_list, x_attention_encoder_all_stages):
        ...
        decoder_output, _ = self.decoder(
            ...
            x_attention_encoder_all_stages=x_attention_encoder_all_stages,
        )
    ...
    def forward(self, x):
        ...
        """
        Variable: x_attention_encoder_all_stages:
        [
        [layer.0.block.0, layer.0.block.1],
        [layer.1.block.0, layer.1.block.1],
        [layer.2.block.0, layer.2.block.1, layer.2.block.2, layer.2.block.3, layer.2.block.4, layer.2.block.5],
        [layer.3.block.0, layer.3.block.1],
        ]
        """

        x, concat_feature_list, x_attention_encoder_all_stages = self.forward_features(x)
        x = self.forward_up_features(x, concat_feature_list, x_attention_encoder_all_stages)
        x = self.up_x4(x)
